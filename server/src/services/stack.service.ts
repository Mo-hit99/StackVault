...
export const StackService = {
  async saveCommits(repoId: number, authorId: number, commits: any[]): Promise<Commit[]> {
    ...
    for (const c of orderedCommits) {
      ...
      const created = await CommitModel.create({
        id: c.id,
        repoId,
        message: (c.message || '').replace(/\0/g, ''),
        parentId: c.parent_id || c.parent,
        authorId,
  ...
  async getCommitStackPaginated(repoId: number, page: number, limit: number) {
    const offset = (page - 1) * limit;
    const commits = await CommitModel.findByRepoIdPaginated(repoId, limit, offset);
    const total = await CommitModel.countByRepoId(repoId);
